...
st.set_page_config(page_title="Top AWS Repos", layout="wide")
...
st.markdown("<h1>Top Curated AWS Repos</h1>", unsafe_allow_html=True)
st.markdown('<div class="subtitle">Popular GitHub projects related to AWS (cloud tooling, examples, infra-as-code).</div>', unsafe_allow_html=True)
...
def fetch_aws_repos(per_page: int = 30, token: Optional[str] = None):
    # Search for repos mentioning "aws" in name, description or readme — sorted by stars
    q = 'aws in:name,description,readme'
...
items = fetch_aws_repos(per_page=per_page, token=secret_token)
